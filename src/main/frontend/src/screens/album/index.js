...
export default function Album() {
                ...
                for (const [group, musicList] of Object.entries(groupedMusic)) {
                    ...
                    if (existingAlbum) {
                        const existingMusicIds = new Set(
                            existingAlbum.musics.map((music) => music.id)
                        );
                        const newMusicIds = musicList
                            .map(({ id }) => id)
                            .filter((id) => !existingMusicIds.has(id));
    ...
    const fetchAlbumArt = async (albumId) => {
        try {
            ...
            setAlbums((prevAlbums) =>
                prevAlbums.map((album) =>
                    album.id === albumId ? { ...album, cover: imageUrl } : album
                )
            );
        } catch (error) {
    ...
    const fetchAndUploadAlbumCover = async (albumId) => {
        try {
            ...
            setAlbums((prevAlbums) =>
                prevAlbums.map((album) =>
                    album.id === albumId ? { ...album, cover: uploadedAlbumCover.cover } : album
                )
            );
        } catch (error) {
    ...
    const handleDeleteMusic = async (albumId, musicId) => {
        try {
            ...
            setAlbums((prevAlbums) =>
                prevAlbums.map((album) => (album.id === albumId ? updatedAlbum : album))
            );
        } catch (error) {
    ...
    const handleAlbumArtUpload = async (albumId, file) => {
        try {
            ...
            setAlbums((prevAlbums) =>
                prevAlbums.map((album) =>
                    album.id === albumId ? { ...album, cover: URL.createObjectURL(file) } : album
                )
            );
                        ...
                        <div key={album.id} className="album-card-container">
                            <div className="album-card">
                                ...
                                {selectedAlbum === album.id && (
                                    <div className="album-songs">
                                        ...
                                        <div>
                                            ...
                                            <button
                                                onClick={() =>
                                                    handleAddMusicToAlbum(album.id, newMusicId)
                                                }>
                                                Add Music
                                            </button>
                                        </div>
                                ...
                                <div className="album-menu2">
                                    <CiMenuKebab
                                        className="svg-album-menu2"
                                        onClick={() => handleToggleMenu(album.id)}
                                    />
                                    {showMenu === album.id && (
                                        <div className="menu-options">
                                            <p id="p-edit" onClick={() => setEditAlbumId(album.id)}>
                                                정보 수정
                                            </p>
                                            <p
                                                id="p-delete"
                            ...
                            <h3 className="album-h3" onClick={() => handleAlbumClick(album)}>
                            ...
                            {editAlbumId === album.id && (
                                <div>
                                    ...
                                    />
                                    <button onClick={() => handleUpdateAlbumName(album.id)}>
                                        Update
                                    </button>
                                    {/* 앨범 아트 업데이트 양식 추가 */}
                                    <div>
